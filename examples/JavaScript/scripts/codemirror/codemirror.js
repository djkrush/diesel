...
var CodeMirror = (function() {
    ...
    function CodeMirror (place, givenOptions) {
        ...
        if (!gecko) {
            connect(wrapper, "contextmenu", operation(onContextMenu));
        }
        connect(code, "dblclick", operation(onDblClick));
        connect(wrapper, "scroll", function() {
            updateDisplay([]);
            if (options.onScroll) {
        ...
        connect(wrapper, "dragenter", function(e) {
            e.stop();
        });
        connect(wrapper, "dragover", function(e) {
            e.stop();
        });
        connect(wrapper, "drop", operation(onDrop));
        connect(wrapper, "paste", function() {
            focusInput();
            fastPoll();
        });
        ...
        if (targetDocument.activeElement == input) {
            onFocus();
        }
        else {
            onBlur();
        ...
        var instance = {
            ...
            getSearchCursor: function(query, pos, caseFold) {
                return new SearchCursor(query, pos, caseFold);
            },
            markText: operation(function(a, b, c) {
                return operation(markText(a, b, c));
            }),
        ...
        function onKeyDown (e) {
            ...
            if (!options.readOnly) {
                ...
                if (!anyMod && code == 9 && handleTab(e.e.shiftKey)) {
                    return e.stop();
                } // tab
                if (mod && code == 90) {
                    undo();
                ...
                if (mod && ((e.e.shiftKey && code == 90) || code == 89)) {
                    redo();
                    return e.stop();
                } // ctrl-shift-z, ctrl-y
            }
        ...
        function onKeyPress (e) {
            ...
            else if (!e.e.ctrlKey && !e.e.altKey && !e.e.metaKey && code == 9 && options.tabMode != "default") {
                e.stop();
            }
            else {
                fastPoll(curKeyId);
        ...
        function unredoHelper (from, to) {
            ...
            if (change) {
                ...
                for (var i = change.start; i < end; ++i) {
                    replaced.push(lines[i].text);
                }
        ...
        function updateLinesNoUndo (from, to, newText, selFrom, selTo) {
            ...
            for (var i = from.line; i < to.line; ++i) {
                if (lines[i].text.length == maxLineLength) {
        ...
        function scrollIntoView (x1, y1, x2, y2) {
            ...
            if (y1 < screentop) {
                wrapper.scrollTop = Math.max(0, y1 - 2 * lh);
                scrolled = true;
            }
            else if (y2 > screentop + screen) {
                wrapper.scrollTop = y2 + lh - screen;
                scrolled = true;
            }
        ...
        function updateGutter () {
            ...
            if (pad) {
                firstNode.insertBefore(targetDocument.createTextNode(pad), firstNode.firstChild);
            }
        ...
        function indentLine (n, how) {
            ...
            if (how == "prev") {
                if (n) {
                    indentation = lines[n - 1].indentation();
                }
                else {
                    indentation = 0;
                }
                ...
                indentation = mode.indent(state, line.text.slice(curSpaceString.length));
            }
            else if (how == "add") {
        ...
        function markText (from, to, className) {
            ...
            return function() {
                ...
                if (start != null) {
                    changes.push({from: start, to: end + 1});
                }
            };
        }
        ...
        function lineHeight () {
            ...
            if (nlines) {
                return (lineDiv.offsetHeight / nlines) || 1;
            }
            ...
            if (typeof query != "string") // Regexp match
            {
                ...
                if (target.length == 1) {
                    ...
                    this.matches = function(reverse, pos) {
                        ...
                        for (; ;) {
                            if (reverse ? !ln : ln == lines.length - 1) {
                                return;
                            }
        ...
        SearchCursor.prototype = {
                ...
                for (; ;) {
                    ...
                    if (reverse) {
                        ...
                        if (pos.line == lines.length - 1) {
                            return savePosAndFail(lines.length);
                        }
            ...
            from: function() {
                if (this.atOccurrence) {
                    return copyPos(this.pos.from);
                }
            },
            to: function() {
                if (this.atOccurrence) {
                    return copyPos(this.pos.to);
                }
            }
        };
